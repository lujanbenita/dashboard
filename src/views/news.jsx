  ...
  useEffect(() => {
    ;(async () => {
      const res = await fetch(
        `http://api.mediastack.com/v1/news?access_key=${API_MEDIASTACK}&countries=us&languages=en&sort=popularity&limit=35`
      )
      if (res.status === 200) {
      ...
      {newsData.length === 0 ? (
        ...
        <div className="news">
          <div className="news__head-news">
            {arrayNewsHead.map(el => (
              <HeadNewsItem news={el} key={el.title} />
            ))}
          </div>

          <div className="news__big-news">
            {arrayNewsBig.map(el => (
              <BigNewsItem news={el} key={el.title} />
            ))}
          </div>

          <div className="news__content">
            <div className="news__main">
              {arrayNewsMain.map(el => (
                <MainNewsItem news={el} key={el.title} />
              ))}
            </div>
            <div className="news__little">
              {arrayNewsMainLittle.map(el => (
                <LittleNewsItem news={el} key={el.title} />
              ))}
            </div>
          </div>
        </div>
